...
time_needed = 1380 #23h in minutes, 1440 is 24h in min 
data_sets = [ds14,ds15,ds16]
variables = ["ta","tas","ts"]
...
def combine_files(data_sets,variables,time_needed):
    ...
    for variable in variables:
        ...
        for dataset in data_sets:
            total.append(np.array(dataset[variable][index_start:index_end]))
        data[variable] = np.concatenate(total)
    return data

###############################################################################################
data = combine_files(data_sets,variables)
